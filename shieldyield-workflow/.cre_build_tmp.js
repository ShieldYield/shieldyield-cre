...
function readAdapterSnapshot(runtime2, evmClient, adapterName, adapterAddress, abi) {
  const apyCallData = encodeFunctionData({
    abi,
    functionName: "getCurrentAPY"
  });
  const apyResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: adapterAddress,
      data: apyCallData
  ...
  const healthResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: adapterAddress,
      data: healthCallData
  ...
  const breakdownResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: adapterAddress,
  ...
  return {
    ...
    apy,
    isHealthy,
    principal: breakdown[0],
    accruedYield: breakdown[1]
  };
}
function readAllAdapters(runtime2, chainName, addresses) {
  ...
  for (const cfg of adapterConfigs) {
    try {
      const snapshot = readAdapterSnapshot(runtime2, evmClient, cfg.name, cfg.address, cfg.abi);
      snapshots.push(snapshot);
      runtime2.log(`✅ ${cfg.name}: balance=${snapshot.balance}, apy=${snapshot.apy}, healthy=${snapshot.isHealthy}`);
    } catch (err) {
      runtime2.log(`❌ Error reading ${cfg.name}: ${err}`);
    }
  }
  return snapshots;
}
...
function fetchTeamWalletData(sendRequester, teamWalletUrl) {
  try {
    const resp = sendRequester.sendRequest({
      url: teamWalletUrl,
      method: "GET",
    ...
    const balanceEth = Number(balanceWei) / 1000000000000000000;
    return {
      balanceEth,
...
function fetchAllOffchainSignals(runtime2, config) {
  ...
  }))(config.goPlusUrl).result();
  runtime2.log(`\uD83D\uDD12 Security: honeypot=${security.isHoneypot}, openSource=${security.isOpenSource}, proxy=${security.isProxy}`);
  const teamWallet = httpClient.sendRequest(runtime2, fetchTeamWalletData, ConsensusAggregationByFields({
    balanceEth: median,
...
function fetchDefiMetrics(runtime2, defiMetricsUrl) {
  const httpClient = new ClientCapability2;
  const result = httpClient.sendRequest(runtime2, fetchDefiMetricsData, ConsensusAggregationByFields({
    aave: identical,
    compound: identical
  }))(defiMetricsUrl).result();
  return result;
}
...
function computeRiskScore(adapter, currentRisk, offchain) {
  ...
  if (metrics) {
    const util3 = adapter.name.includes("Aave") ? metrics.aave?.utilization : adapter.name.includes("Compound") ? metrics.compound?.utilization : undefined;
    if (util3 !== undefined) {
      if (util3 > 95)
        score += 5;
      else if (util3 > 85)
...
function getThreatLevelLabel(score) {
  if (score <= 25)
    return "SAFE";
  if (score <= 50)
    return "WATCH";
...
function detectAnomalies(adapter, offchain) {
  ...
  if (offchain.github.lastPushDaysAgo > 30 && offchain.teamWallet.recentLargeOutflows) {
    anomalies.push({
      type: "TEAM_EXIT",
      severity: "CRITICAL",
      adapter: adapter.name,
  ...
  const util3 = adapter.name.includes("Aave") ? offchain.defiMetrics?.aave?.utilization : adapter.name.includes("Compound") ? offchain.defiMetrics?.compound?.utilization : undefined;
  if (util3 !== undefined && util3 > 95) {
    anomalies.push({
      type: "LIQUIDITY_CRUNCH",
      severity: "CRITICAL",
      adapter: adapter.name,
      message: `Protocol utilization at ${util3.toFixed(1)}% — liquidity crunch imminent`
    });
  } else if (util3 !== undefined && util3 > 85) {
    anomalies.push({
...
${"=".repeat(60)}`);
  ...
  let defiMetrics = { aave: null, compound: null };
  try {
    const defiMetricsUrl = apisConfig.defiMetricsUrl;
    if (defiMetricsUrl) {
      runtime2.log("Fetching DeFi protocol metrics (AAVE/Compound)...");
      defiMetrics = fetchDefiMetrics(runtime2, defiMetricsUrl);
      if (defiMetrics.aave) {
        runtime2.log(`\uD83D\uDCCA AAVE: supplyAPY=${defiMetrics.aave.supplyApy}%, borrowAPY=${defiMetrics.aave.borrowApy}%, util=${defiMetrics.aave.utilization}%`);
      }
      if (defiMetrics.compound) {
        runtime2.log(`\uD83D\uDCCA Compound: supplyAPR=${defiMetrics.compound.supplyApr}%, borrowAPR=${defiMetrics.compound.borrowApr}%, util=${defiMetrics.compound.utilization}%`);
      }
    }
  } catch (err) {
    runtime2.log(`⚠️ DeFi metrics unavailable: ${err}`);
  }
  const offchain = {
...
var onRebalanceTrigger = (runtime2, triggerEvent) => {
  ...
  runtime2.log("=".repeat(60));
  const evm = runtime2.config.evms.find((e) => e.chainName === "ethereum-testnet-sepolia-arbitrum-1") || runtime2.config.evms[0];
  const addresses = evm.addresses[0];
  const evmClient = createEvmClient(evm.chainName);
...
var onShieldTrigger = (runtime2, triggerEvent) => {
  ...
  try {
    if (triggerEvent?.topics && triggerEvent.topics.length >= 2) {
      const addrHex = typeof triggerEvent.topics[1] === "string" ? triggerEvent.topics[1] : toHex(triggerEvent.topics[1]);
